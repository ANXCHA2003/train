...
BATCH_SIZE = 32  # เพิ่มขึ้นเพื่อให้การเรียนรู้เสถียรขึ้น
...
INITIAL_EPOCHS = 30  # ลดลงเพื่อทดสอบ
...
class_names = sorted([os.path.basename(d) for d in glob.glob(os.path.join(DATA_DIR, '*')) if os.path.isdir(d)])

if not class_names:

    raise ValueError(f"ไม่พบคลาสในโฟลเดอร์ {DATA_DIR}")
